...
class BPETokenizer:
    ...
    def train(self, text):
        ...
        word_counts = collections.Counter(word_tokenize(text))
        vocab_counts = {' '.join(list(word) + ["▁"]): count for word, count in word_counts.items()}
        # vocab_counts example: {'t h e ▁': 10, ...}
        ...
        num_merges = self.vocab_size - len(self.special_tokens) - 256 # Approximate subtraction for base chars? 
        # Actually proper BPE stops when vocab size is reached.
        # But here we build vocab AFTER merges. 
        # Typical BPE: start with chars, add merges one by one. Each merge adds 1 to vocab size.
        # Let's count initial vocab size (unique chars).
        
        # Initial unique tokens
        unique_tokens = set()
        for word in vocab_counts:
            unique_tokens.update(word.split())
        
        # We want final vocab size to be self.vocab_size.
        # Currently we have len(unique_tokens) + len(self.special_tokens).
        # We can add (self.vocab_size - current_size) merges.
        
        while len(unique_tokens) + len(self.special_tokens) < self.vocab_size:
            pairs = self.get_stats(vocab_counts)
            if not pairs:
                break
                
            best_pair = max(pairs, key=pairs.get)
            self.merges.append((best_pair, ''.join(best_pair)))
            self.bpe_ranks[best_pair] = len(self.bpe_ranks)
            
            vocab_counts = self.merge_vocab(best_pair, vocab_counts)
            
            # Update unique tokens? 
            # The new token is ''.join(best_pair).
            # The constituent tokens might still exist in other words, so we add 1 to vocab count.
            unique_tokens.add(''.join(best_pair))
        ...
        # We need all unique tokens from the final vocab_counts keys
        final_tokens = set()
        for word in vocab_counts:
