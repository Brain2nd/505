...
class NGramLM:
    ...
    def train(self, tokens):
        ...
        # We need to collect vocabulary as well for Laplace smoothing
    ...
    def perplexity(self, test_tokens):
        ...
        # We need to handle the start of the sequence.
        # The prompt implies we just iterate through ngrams in test_tokens.
        # We should use the same get_ngrams helper.
        
        ngrams = self.get_ngrams(test_tokens, self.n)
        log_prob_sum = 0
        N = len(ngrams)
        
        # Note: perplexity is usually exp(-1/N * sum(log P(w_i | w_{i-n+1}...w_{i-1})))
        # Make sure to handle unknown vocab if necessary, but NGram logic typically handles unseen ngrams via smoothing.
        # However, unseen words in 'token' position are not explicitly handled by 'vocab' set update here (frozen after train).
        # But get_prob doesn't check vocab membership, just uses len(self.vocab).
...
class RNNLM(nn.Module):
    ...
    def forward(self, x, hidden=None):
        ...
        output_tensor = torch.stack(outputs, dim=1) # (Batch, Seq, Hidden)
        logits = self.fc(output_tensor)
...
class LSTMLM(RNNLM):
    ...
    def lstm_cell(self, x, h_prev, c_prev):
        ...
        # The chunks are usually in order: i, f, g, o or i, f, o, g depending on implementation. 
        # PyTorch default is i, f, g, o.
...
if __name__ == "__main__":
    ...
    words = ["the", "cat", "sat", "on", "the", "mat", "unknowntoken123"]
    print("Manual Tokenization Check:")
    for w in words:
        print(f"Tokenizing '{w}': {tokenizer.tokenize(w)}")
